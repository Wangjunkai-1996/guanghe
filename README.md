...
- 当前前端 UI 最新基线说明：`docs/frontend-ui-v5-plan.md`

## 前端 UI 当前基线（2026-03-15 / V5）

为避免后续继续混淆版本号，README 统一约定：

- 当前前端 UI 最新版本统一记为 `V5`
- 2026-03-15 这轮完成的“恢复上一轮偏好版本、桌面端暗色控制台化、Google Cloud 风格收口、按钮可读性修复”全部归入 `V5`
- 后续如果继续做前端页面优化，应默认以 `V5` 为最新基线；`V1 / V2 / V3 / V4` 视为历史阶段
...
- 工作区固定为：
  - `批量闭环`
  - `快速验证`
- 桌面端默认采用暗色高密工具台，而不是展示型后台或品牌化 Hero 页面
- 批量页固定为四阶段工作流：
  1. `交接表准备`
  2. `需求摘要`
  3. `任务发起`
  4. `任务队列`
- 手工页固定为快速验证工具：左侧账号 rail，右侧查询条与结果面板
- 顶部全局状态统一只保留：
  - 当前工作区
  - 账号库状态
  - 腾讯文档状态
...
- 恢复用户偏好的桌面端暗色 VNext 工具台，不再回退到更早的浅色/品牌化页面
- 修正桌面端多个按钮文字颜色继承错误，避免按钮背景与按钮文案冲突
- 将提示卡、状态条、侧栏状态模块统一收进暗色体系
- 将桌面端壳层进一步收口为更接近 `Google Cloud 控制台` 的风格：石墨灰底、低饱和蓝强调、分层面板、克制动效
...
- 支持两种工作模式：
  - `批量闭环`：围绕交接表、二维码任务、查询与腾讯文档回填组织主链路
  - `快速验证`：手动选择已保存账号并发起单次查询，保留最近结果做复核
- 同一账号查询串行执行，不同批量任务支持受控并发
- 页面内发起扫码登录，并展示登录后的账号昵称/头像
- 当前前端 UI 已统一进入 `V5` 基线：批量页按四阶段重排，手工页降级为快速验证工具，桌面端默认使用暗色控制台样式
- 固定查询 `30日` 和以下 5 个指标：
  - `内容查看次数`
  - `内容查看人数`
  - `种草成交金额`
